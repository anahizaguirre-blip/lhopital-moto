...
import type { Product, CrossSell, ProductVariant } from '@/lib/types';
...
export function ProductDetail({ product, crossSells }: ProductDetailProps) {
  // Galería: extrae basePath y genera las 4 vistas
  const basePath = extractBasePath(product.imagen_principal);
  const gallery = basePath
    ? hedonGallery(basePath)
    : null;
    ...
    <section className="px-6 md:px-12 lg:px-24 pb-12">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
        ...
        <div className="lg:pt-4">
          ...
          {/* Banner de bajo pedido si no hay stock */}
      ...
      {/* Cross-sells */}
      {crossSells.length > 0 && (
        <div className="max-w-6xl mx-auto mt-24 pt-12 border-t border-[#F4F1EC]/8">
          <div className="mb-3 flex items-center gap-2">
            <span className="inline-block w-6 h-px bg-[#C9A961]" />
            <span className="text-[10px] tracking-[0.3em] uppercase text-[#C9A961]">
              Para tu {product.familia}
            </span>
          </div>
          <h2 className="text-2xl md:text-3xl font-medium tracking-tight mb-2">
            Personaliza tu casco.
          </h2>
          <p className="font-cormorant italic text-base text-[#F4F1EC]/55 mb-8">
            El detalle que solo notan los conocedores.
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {crossSells.slice(0, 6).map((cs) => {
              if (!cs.suggested_product) return null;
              const sp = cs.suggested_product;
              return (
                <div
                  key={cs.id}
                  className="bg-[#F4F1EC]/3 border border-[#F4F1EC]/10 p-4 flex gap-3 items-center"
                >
                  <div className="w-16 h-16 bg-[#1a1a1a] flex-shrink-0 relative overflow-hidden">
                    <Image
                      src={cloudinaryUrl(sp.imagen_principal, 'thumbnail')}
                      alt={sp.nombre}
                      fill
                      sizes="64px"
                      className="object-cover"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-[13px] mb-0.5 truncate">
                      {sp.nombre}
                    </div>
                    <p className="text-[11px] text-[#F4F1EC]/55 leading-snug mb-1 line-clamp-2">
                      {cs.mensaje}
                    </p>
                    <div className="text-[12px] text-[#C9A961] font-medium">
                      + ${sp.precio_base.toLocaleString('es-MX')}
                    </div>
                  </div>
                </div>
              );
